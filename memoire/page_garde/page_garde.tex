%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\documentclass[12pt,a4paper,oneside]{article}
\usepackage{times}
\usepackage{amsmath}
\usepackage{amsfonts}
\usepackage{amssymb}
\usepackage{graphicx}
\usepackage{fancyhdr}
\usepackage{tabularx}
\usepackage{url}

\usepackage[frenchb]{babel} 
\usepackage[utf8]{inputenc}
\usepackage[T1]{fontenc}

%\pagestyle{headings}

%\setlength{\hoffset}{-58pt}
\setlength{\hoffset}{-42.2pt} %DECALAGE POUR RELIURE  	
\setlength{\oddsidemargin}{0pt} 	% Marge gauche sur pages impaires
\setlength{\evensidemargin}{0pt} % Marge gauche sur pages paires
\setlength{\textwidth}{570pt} 	% Largeur de la zone de texte (17cm)
\setlength{\textheight}{845pt} 
\setlength{\topmargin}{-115pt} 

\setlength{\footskip}{30pt}

\setlength{\headsep}{40pt} 
\setlength{\headheight}{0pt} 
\setlength{\marginparsep}{0pt}

\begin{document}
\begin{titlepage}
\vspace*{0.95cm}
\begin{center}
\textnormal{\Large{Universit\'e de Mons}}\\[0.3em]
\textnormal{\Large{Facult\'e des Sciences}}\\[0.3em]
\textnormal{\Large{D\'epartement d'Informatique}}\\[0.3em]
\textnormal{\Large{Réseaux et Télécommunications}}
\end{center}
\vspace*{4cm}
\begin{center}
\fbox{
\begin{minipage}{16cm}
\center
\vspace*{0.5cm}\textbf{\LARGE{Conception et évaluation d'une architecture hybride de réseaux de capteurs}}\\[0.5em]
\textbf{\LARGE{reposant sur les technologies radio LoRa et IEEE 802.15.4}}\vspace*{0.5cm}
\end{minipage}
}
\end{center}
\vspace*{3cm}

\large{
    \begin{center}
        \begin{tabular*}{16.7cm}{@{\extracolsep{\fill}}lr}
            Directeur : M\textsuperscript{r} Bruno \textsc{Quoitin} & M\'emoire r\'ealis\'e par\\
            & Arnaud \textsc{Palgen}\\[1em]
            Rapporteurs : M\textsuperscript{r} Pr\'enom \textsc{NOM} & en vue de l'obtention du grade de\\
        \hspace{26.4mm}M\textsuperscript{r} Pr\'enom \textsc{Nom} & Master en Sciences Informatiques
        \end{tabular*}
    \end{center}}

\vspace*{4cm}
\begin{center}
    \includegraphics[height=2cm]{../res/pictures/UMONS-logo.jpg}
    \hspace{3cm}
    \includegraphics[height=1.7cm]{../res/pictures/FS-logo.jpg}
    \\[1em]
    Ann\'ee acad\'emique 2020-2021
\end{center}

\end{titlepage}

\end{document}