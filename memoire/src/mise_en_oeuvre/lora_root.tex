...
\renewcommand{\rightmark}{Racine LoRa}

    
    Dans la topologie choisie à la section \ref{sec:archi-topologie}, une racine LoRa est le RN2483 
    interfacé à un Raspberry Pi. Pour le développement, le RN2483 est interfacé à un ordinateur 
    portable. Que ce soit à l'ordinateur portable ou au raspberry pi, la carte d'interface du 
    RN2483 est raccordée en USB via un bridge USB-UART.

    L'implémentation se présente sous forme d'un package python qui donne accès à une variable 
    \texttt{NETWORK\_STACK} qui fait référence à une instance de la classe \texttt{NetworkStack} et 
    permet d'accèder aux trois 
    différentes couches, l'adresse LoRaMAC du noeud, son adresse IPv6 issue de la conversion de son 
    adresse LoRaMAC ainsi que trois fonctions: \texttt{init}, \texttt{send} et \texttt
    {register\_listener} qui sont des 
...
\subsection*{Couche physique}
    ...
    {chap:archi}. Ces deux classes possèdent les fonctions nécessaires pour sérialiser les trames LoRamAC (resp.désérialiser) en (resp. depuis) une chaîne de caractères hexadecimal qui est le format de donnée demandé par le RN2483.
...
\subsection*{Couche MAC}
    ...
    Pour la suite du mémoire, $sf$ est défini comme le numéro de séquence d'une trame reçue et $se$ 
    ...
    \subsubsection*{Fonctions publiques de la couche MAC}
